...
/// Checks if the king of the side whose turn it is to move is in check.
BOOLTYPE isOtherKingAttacked() {
  // If it's White's turn to move, check if the white king is attacked by black.
  if (board.nextMove == WHITE_MOVE) {
    if (board.whiteKing == 0) return true;
    return isAttacked(firstOne(board.whiteKing), BLACK_MOVE);
  } else {
    // If it's Black's turn to move, check if the black king is attacked by white.
    if (board.blackKing == 0) return true;
    return isAttacked(firstOne(board.blackKing), WHITE_MOVE);
  }
}
